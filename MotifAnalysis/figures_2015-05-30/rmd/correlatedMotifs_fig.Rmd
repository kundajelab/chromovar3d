...
inrmd='/srv/gsfs0/projects/kundaje/users/oursu/code/sequencingUtilities/peakCalling/chromoVariationCode/MotifAnalysis/figures_2015-05-30/rmd/correlatedMotifs_fig.Rmd'
...
browserTrack=function(datain,out){
  ...
  for (m in motifs){
    ...
    data=paste('chr',chromo,':',snppos,'-',snppos+1,
...
bedfile_correlated_motifs=function(datain,out,sig_threshold){
  ...
  for (m in motifs){
    ...
    write_var=unique(paste(datacur[,CHR_COL],datacur[,SNPPOS_COL],as.numeric(as.character(datacur[,SNPPOS_COL]))+1,m,sep='\t'))
    write_sig=unique(paste(datacur_sig[,CHR_COL],datacur_sig[,SNPPOS_COL],as.numeric(as.character(datacur_sig[,SNPPOS_COL]))+1,m,sep='\t'))
  ...
  for (hmark in hmarks_affected){
    ...
    write_var=unique(paste(datacur[,CHR_COL],datacur[,SNPPOS_COL],as.numeric(as.character(datacur[,SNPPOS_COL]))+1,datacur$motifName,sep='\t'))
    write_sig=unique(paste(datacur_sig[,CHR_COL],datacur_sig[,SNPPOS_COL],as.numeric(as.character(datacur_sig[,SNPPOS_COL]))+1,datacur_sig$motifName,sep='\t'))
...
#here we aggregate data only across QTL peaks
...
#TODO: run these
browserTrack(total_local,paste(dirname(INFILE_LOCAL),'/LocalMotifTracks/motifCorr',sep=''))
browserTrack(total_distal,paste(dirname(INFILE_DISTAL),'/DistalMotifTracks/motifCorr',sep=''))
bedfile_correlated_motifs(total_local,paste(dirname(INFILE_LOCAL),'/SNPsInMotifsBed/Local/_',sep=''),0.05)
...
first=TRUE
for (hmark in hmarks){
  fraction_cur=fractionsPlot(motif_datasets_local_all[[hmark]],SIG_THRESH,paste(INFILE_LOCAL,'FractionPlot',hmark,sep=''),hmark,MIN_N,10)
  if (first==FALSE){
    fractionTotal=rbind(fraction_cur,fractionTotal)
      }
  if (first==TRUE){
    fractionTotal=fraction_cur
    first=FALSE
  }
}
print(fractionTotal)
print(ggplot(fractionTotal,aes(x=TF,y=100*value,fill=CorrSign))+facet_wrap(~hmark)+guides(fill=guide_legend(title='Correlation sign'))+scale_fill_manual(values=c('black','lightblue'))+geom_bar(stat="identity")+coord_flip() +theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(), 
    panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))+ylab('Percent tested motif instances \nthat correlate with histone mark')+xlab('')+ggtitle(hmark))

#fractionsPlot(total_local_all,SIG_THRESH,paste(INFILE_LOCAL,'FractionPlot',hmark,sep=''),hmark,MIN_N,10)
```
