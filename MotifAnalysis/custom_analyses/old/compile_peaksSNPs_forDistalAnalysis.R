...
newqtl_brief=newqtl[,c('chr','gene','peak2.id')]
newqtl_with_overlappingRNA=newqtl_brief
unique_peak1s=as.character(unique(newqtl_brief$gene))
pcounter=0
for (p1 in unique_peak1s){
    pcounter=pcounter+1
    print(pcounter)
    p1.rows=newqtl[which(as.character(newqtl$gene)==p1),]
    p1.range=GRanges(seqnames =Rle(p1.rows[1,'chr']), ranges =IRanges(p1.rows[1,'peak1.start'], end = p1.rows[1,'peak1.end']))
    ...
    if (length(matching.rnas)>0){
       for (rna.item in c(1:length(matching.rnas))){
       	   ...
       	   combinedRows=p1.rows[,c('chr','gene','peak2.id')]
       	   combinedRows[,'gene']=rna.names
	   print(combinedRows)
...
write.table(newqtl_with_overlappingRNA,file='/srv/gs1/projects/snyder/jzaugg/histoneQTL/motif_analysis/results/2014-07-28/HiC_local_QTL_dist_QTL.all.txt_augmentPeak1Peak2IDs.ChrGenePeak2.PlusOverlappedRNA.txt',sep='\t',quote=F,row.names=F,col.names=T)
